...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        #self._scores = torch.bmm(torch.log(eps+F.relu(queries)), torch.log(eps+F.relu(keys.transpose(1, 2)))) / np.sqrt(K)
        #self._scores = torch.bmm(torch.log(eps+F.relu(queries)), keys.transpose(1, 2)) / np.sqrt(K)
        #self._scores = torch.bmm(torch.log(eps+F.softmax(queries,dim=-1)), F.softmax(keys.transpose(1, 2),dim=-2)) / np.sqrt(K)
